...
It's the web's perfect union of Symfony (Doctrine/PHP), Node.js/Webpack (JavaScript), and Bootstrap (SCSS). It is a heavily database driven system that handles domains, subdomains, routing, Electron applications, Webpack bundles, client-side routing, it's... well, it's built on Webpack Encore and it's awesome. It's better than anything that's out there - except for Drupal.
